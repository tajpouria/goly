...
func (s *Suite) SetupSuite() {
	...
	s.DB, err = gorm.Open(dialector, &gorm.Config{
		Logger: logger.Default.LogMode(logger.Silent), // Use logger.Info for more verbosity
	})
	require.NoError(s.T(), err)
...
func (s *Suite) TestUpdateGoly() {
	...
	s.mock.ExpectBegin()
	s.mock.
		ExpectExec(regexp.QuoteMeta(`UPDATE "golies" SET "redirect"=$1,"goly"=$2,"clicked"=$3,"random"=$4 WHERE "id" = $5`)).
		WithArgs(redirect, goly, clicked, random, id).
...
func (s *Suite) TestDeleteGoly() {
	...
	s.mock.ExpectBegin()
	s.mock.
		ExpectExec(regexp.QuoteMeta(`DELETE FROM "golies" WHERE "golies"."id" = $1`)).
		WithArgs(id).
		WillReturnResult(sqlmock.NewResult(int64(id), 1))
	s.mock.ExpectCommit()

	err := s.repo.DeleteGoly(&Goly{
		ID:       id,
		Redirect: redirect,
		Goly:     goly,
		Clicked:  clicked,
